...
export function trimTokens(context, maxTokens, model) {
    // Function to trim text to fit within a token limit for language models
    
    // Get the appropriate tokenizer for the specified model
    const encoding = tiktoken.encoding_for_model(model as TiktokenModel);
    
    // Convert the text context into tokens
    let tokens = encoding.encode(context);
    
    // Create a decoder to convert tokens back to text
    const textDecoder = new TextDecoder();
    
    // If we have more tokens than allowed
    if (tokens.length > maxTokens) {
        // Reverse array, slice off excess tokens from end, then reverse back
        // This keeps the most recent tokens rather than earliest ones
        tokens = tokens.reverse().slice(maxTokens).reverse();

        // Convert the trimmed tokens back into text
        context = textDecoder.decode(encoding.decode(tokens));
    }
    
    // Return either the original or trimmed context
    return context;
}
/**
